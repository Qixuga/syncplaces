/* ***** BEGIN LICENSE BLOCK *****
 *
 * The code in this file was taken from the following web page on 10/10/2008:
 * http://www.movable-type.co.uk/scripts/aes.html
 *
 * I've encapsulated all the functions and variables to prevent namespace
 * collisions with other extensions and removed the extension of String
 * with prototypes as this is not allowed in Firefox
 *
 ...
 *
 * Copyright (C) 2005-2008 Chris Veness
 *
...
var SyncPlacesAES = {
	...
	Cipher: function(input, w) {    // main Cipher function [5.1]
		...
		function SubBytes(s, Nb) {    // apply SBox to state S [5.1.1]
			for (var r=0; r<4; r++) {
				for (var c=0; c<Nb; c++) s[r][c] = SyncPlacesAES.Sbox[s[r][c]];
			}
			return s;
		}
	...
	KeyExpansion: function(key) {  // generate Key Schedule (byte-array Nr+1 x Nb) from Key [5.2]
		...
		for (var i=Nk; i<(Nb*(Nr+1)); i++) {
			...
			if (i % Nk == 0) {
				...
				for (var t=0; t<4; t++) temp[t] ^= SyncPlacesAES.Rcon[i/Nk][t];
			} else if (Nk > 6 && i%Nk == 4) {
				temp = SubWord(temp);
			}
		...
		function SubWord(w) {    // apply SBox to 4-byte word w
			for (var i=0; i<4; i++) w[i] = SyncPlacesAES.Sbox[w[i]];
			return w;
		}
	...
	AESEncryptCtr: function(plaintext, password, nBits) {
		...
		plaintext = SyncPlacesEncryptUtils.encodeUTF8(plaintext);
		password = SyncPlacesEncryptUtils.encodeUTF8(password);
		...
		ciphertext = SyncPlacesEncryptUtils.encodeBase64(ciphertext);  // encode in base64
	...
	AESDecryptCtr: function(ciphertext, password, nBits) {
		var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
		if (!(nBits==128 || nBits==192 || nBits==256)) return '';  // standard allows 128/192/256 bit keys
		ciphertext = SyncPlacesEncryptUtils.decodeBase64(ciphertext);
		password = SyncPlacesEncryptUtils.encodeUTF8(password);
		//var t = new Date();  // timer
		...
		plaintext = SyncPlacesEncryptUtils.decodeUTF8(plaintext);  // decode from UTF8 back to Unicode multi-byte chars
...
var SyncPlacesEncryptUtils = {
	...
	encodeBase64: function(str, utf8encode) {  // http://tools.ietf.org/html/rfc4648
		utf8encode =  (typeof utf8encode == 'undefined') ? false : utf8encode;
		var o1, o2, o3, bits, h1, h2, h3, h4, e=[], pad = '', c, plain, coded;
  	var b64 = this.base64;

		plain = utf8encode ? this.encodeUTF8(str) : str;
	...
	decodeBase64: function(str, utf8decode) {
  	...
  	var b64 = this.base64;
		...
		coded = utf8decode ? this.decodeUTF8(str) : str;
		...
		return utf8decode ? this.decodeUTF8(plain) : plain;
	},
